...
// 666 Krixes Self Bloodbag Start
    ...
    if((speed player <= 1) && hasBagItem && _canDo) then {
        if (s_player_selfBloodbag < 0) then {
            s_player_selfBloodbag = player addaction[("<t color=""#c70000"">" + ("Self Bloodbag") +"</t>"),"scripts\selfbloodbag.sqf","",5,false,true,"", ""];
        };
    } else {
        player removeAction s_player_selfBloodbag;
        s_player_selfBloodbag = -1;
    };
// 666 end Krixes Self Bloodbag End
// 666 zombiebait-bomb
    ...
    if (!_isVehicle or (_isVehicle and (player distance cursorTarget > 4))) then {
        ...
        if (_hasZombieBaitParts and _canDo and !a_player_customAction) then {
            if (s_player_craftZombieBait < 0) then {
                s_player_craftZombieBait = player addAction[("<t color=""#c70000"">" + ("Craft Zombie Bait") + "</t>"),"scripts\zombiebait.sqf","",2,false,true,"",""];
            };
        } else {
        ...
        if (_hasZombieBombParts and _canDo and !a_player_customAction) then {
            if (s_player_craftZombieBaitBomb < 0) then {
                s_player_craftZombieBaitBomb = player addAction[("<t color=""#c70000"">" + ("Craft Zombie Bait Bomb") + "</t>"),"scripts\zombiebomb.sqf","",2,false,true,"",""];
            };
        } else {
...
if (!isNull cursorTarget and !_inVehicle and !_isPZombie and (player distance cursorTarget < _allowedDistance) and _canDo) then {	//Has some kind of target
    ...
    _arrestreq = "ItemWire" in magazines player;    // CHANGE THIS TO THE ITEM YOU WANT PLAYERS TO NEED TO BE ABLE TO ARREST
	...
	if (_player_studybody) then {
		if (s_player_studybody < 0) then {
			s_player_studybody = player addAction [localize "str_action_studybody", "scripts\studybody.sqf",_cursorTarget, 0, false, true, "",""];
		};
	} else {
    ...
    if (_isMan and !_isAlive and !_isZombie and !_isAnimal) then {
		if (s_clothes < 0) then {
            s_clothes = player addAction [("<t color=""#FF0000"">" + ("Take Clothes") + "</t>"), "scripts\takeclothes.sqf",cursorTarget, 1, false, true, "",""];
        };
    } else {
	...
	if (!_isAlive and !_isZombie and !_isAnimal and _hasETool and _isMan and _canDo) then {
        if (s_player_bury_human < 0) then {
            s_player_bury_human = player addAction [format["Bury Body"], "scripts\burybody.sqf",cursorTarget, 3, true, true, "", ""];
        }
    } else {
...
if (!_isAlive and !_isZombie and !_isAnimal and _isMan and _canDo) then {
	        if (s_player_eat_human < 0) then {
	            s_player_eat_human = player addAction [format["Eat Human Heart"], "scripts\cannibalism.sqf",cursorTarget, 3, true, true, "", ""];
	        }
	    } else {
	...
	*/
    ...
    if (_isMan and _isAlive and !_isZombie and _canDo and !_unconscious) then {
        if (s_player_knockout < 0) then {
            s_player_knockout = player addAction [("<t color=""#FF9800"">" + ("Knockout!") + "</t>"), "scripts\knockout.sqf",cursorTarget, 0, false, true, "",""];
        };
    } else {
...
if (_canDrink) then {
        if (s_player_drinkWater < 0) then {
            s_player_drinkWater = player addaction[("<t color=""#0000c7"">" + ("Drink water") +"</t>"),"scripts\drinkwater.sqf"];
        };
    } else {
